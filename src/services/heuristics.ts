...
export function heuristicFactors(input: RiskRequest) {
  ...
  factors.push({
    label: "largeAmount",
    weight: policyV1.weights.largeAmount,
    value: largeAmount,
  });

  const unusualLocation =
    input.location && input.location !== "Nairobi" ? 1 : 0;
  factors.push({
    label: "unusualLocation",
    weight: policyV1.weights.unusualLocation,
  ...
  factors.push({
    label: "missingUserId",
    weight: policyV1.weights.missingUserId,
    value: missingUserId,
  });
  ...
  factors.push({
    label: "riskyAddress",
    weight: policyV1.weights.riskyAddress,
    value: riskyAddress,
  });
